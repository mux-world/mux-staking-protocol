...
describe("Integration", async () => {
    ...
    it("claim all ex", async () => {
        ...
        await setTime(base + WEEK * 3 + 1000) // week2
        {
            ...
            const {
                mlpFeeAmount, mlpMuxAmount, veFeeAmount, veMuxAmount
            } = await router.callStatic.claimableRewards(user0.address)
        ...
        await setTime(base + WEEK * 4 + 1000) // week2
        {
            ...
            const balance0 = await ethers.provider.getBalance(user0.address)
            const {
                mlpFeeAmount, mlpMuxAmount, veFeeAmount, veMuxAmount, mcbAmount
            } = await router.callStatic.claimableRewards(user0.address)
